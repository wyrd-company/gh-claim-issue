...
func cmdClaim(args []string) error {
	...
	var (
		repoFlag  = fs.String("repo", "", "owner/name (defaults to the current repository)")
	...
	owner, name, err := resolveRepo(*repoFlag)
	if err != nil {
		return err
	}
...
Flags (claim):
  --repo owner/name        target repo (default: current)
...
for the same repository, so only one agent can mutate at a time.
`, adj, noun, adj*noun)
}
